...
contract PoolOpinionMarket is OpinionMarket {
        ...
        if (!isReady) {
            ...
            if (errorCode == 2)
                revert OpinionMarketErrors.ERR(
                    OpinionMarketErrors.POOL_INSUFFICIENT_FUNDS
                );
        }
    ...
    function withdrawFromExpiredPool(
        ...
        if (!isExpired)
            revert OpinionMarketErrors.ERR_DATA(
                OpinionMarketErrors.POOL_NOT_EXPIRED,
                poolId,
                poolInfo.deadline
            );
        ...
        if (userContribution == 0)
            revert OpinionMarketErrors.ERR_DATA(
                OpinionMarketErrors.POOL_NO_CONTRIBUTION,
                poolId,
    ...
    function claimAccumulatedFees() external nonReentrant whenNotPaused {
        ...
        if (amount == 0)
            revert OpinionMarketErrors.ERR(
                OpinionMarketErrors.NO_FEES_TO_CLAIM
            );
