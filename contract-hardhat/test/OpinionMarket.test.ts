...
describe("OpinionMarket", () => {
    ...
    describe("Rate Limiting", () => {
        ...
        it("should prevent multiple trades in same block by same user", async () => {
            // First trade
            await opinionMarket.connect(user1).submitAnswer(1, "Answer 1");
        ...
        it("should allow different users to trade in same block", async () => {
            await opinionMarket.connect(user1).submitAnswer(1, "First Answer");
            await opinionMarket.connect(user2).submitAnswer(1, "Second Answer");

            const history = await opinionMarket.getAnswerHistory(1);
            expect(history.length).to.equal(3); // Initial + 2 answers
        });

        it("should allow same user to trade in different blocks", async () => {
            await opinionMarket.connect(user1).submitAnswer(1, "Answer 1");
