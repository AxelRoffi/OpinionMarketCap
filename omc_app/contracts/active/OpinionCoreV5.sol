...
/**
 * @title OpinionCoreV5
 * @notice Identical to V4 except `createOpinion` now accepts an optional
 *         `link` (source URL) for the creator's bootstrap answer — bringing
 *         it to parity with `submitAnswer`, which already stored `link`.
 * @dev STORAGE-SAFE upgrade: no state variables added, removed, or reordered.
 *      The `Opinion` struct already had a `link` field; V4 simply hard-wrote
 *      it to "" at creation. V5 writes the caller-supplied value instead.
 *
 *      All V4 economics (self-exit, vacant-slot reclaim, locked stake) are
 *      preserved verbatim. Heavy logic still lives in `SelfExitLib`.
 */
contract OpinionCoreV5 is
    Initializable,
    UUPSUpgradeable,
