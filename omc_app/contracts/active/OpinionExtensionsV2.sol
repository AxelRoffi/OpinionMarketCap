...
/**
 * @title OpinionExtensionsV2
 * @dev Extension slots and categories management - Size optimized (~8KB)
 *
 * V2 CHANGES:
 * - Fixed validateCategories() to require at least one category (was allowing empty arrays)
 * - Fixed initializeOpinionCategories() to require at least one category
 */
contract OpinionExtensionsV2 is
    Initializable,
    UUPSUpgradeable,
    ...
    IOpinionMarketErrors
{
    ...
    uint256 public constant MIN_CATEGORIES_PER_OPINION = 1; // V2: Added minimum
    ...
    /**
     * @dev V2 FIX: Now requires at least one category
     */
    function initializeOpinionCategories(uint256 opinionId, string[] memory _categories) external onlyCoreContract {
        // V2 FIX: Require at least one category
        require(_categories.length >= MIN_CATEGORIES_PER_OPINION, "At least one category required");
    ...
    /**
     * @dev V2 FIX: Now requires at least one category (was allowing empty arrays)
     */
    function validateCategories(string[] memory _categories) external view onlyCoreContract returns (bool) {
        // V2 FIX: Require at least one category
        if (_categories.length < MIN_CATEGORIES_PER_OPINION) return false;
