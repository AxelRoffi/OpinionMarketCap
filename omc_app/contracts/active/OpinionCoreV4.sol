...
/**
 * @title OpinionCoreV4
 * @notice Adds self-exit + vacant-slot reclaim on top of V3.
 * @dev α-vanilla economics: creator pre-funds the slot at `initialPrice`;
 *      that amount is locked in the contract and carries forward to each
 *      subsequent king. A stuck king (no trade for `soloCooldown`) may
 *      OPTIONALLY exit and recover `(1 - exitPenaltyBps)` of the locked
 *      stake. After exit the slot becomes vacant and is reclaimable at
 *      `reclaimDiscountBps` of the exiter's last trade price.
 *
 *      Migration policy (Path A): legacy opinions created under V3 keep
 *      `lockedStake = 0`. They cannot self-exit. New opinions created via
 *      V4's `createOpinion` are bootstrapped with a real lock from day 1.
 *
 *      Heavy logic lives in `SelfExitLib` (deployed separately, called via
 *      DELEGATECALL) to keep this contract under Base's 24KB limit.
 */
contract OpinionCoreV4 is
    Initializable,
    UUPSUpgradeable,
    ...
    IValidationErrors
{
    ...
    // should include both ABIs (OpinionCoreV4 + SelfExitLib) when decoding logs.
