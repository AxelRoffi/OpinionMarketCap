...
export interface LeaderboardUser {
  ...
  volumeGenerated: number; // Total trading volume from their questions
  categories: string[]; // Categories the user is active in
}
...
export function useLeaderboardData(filters?: LeaderboardFilters) {
  ...
  const rankingType = filters?.rankingType || 'earnings';

  // Calculate leaderboard data from blockchain
  const leaderboardData = useMemo(() => {
    if (!opinions || opinions.length === 0) {
      return { users: [], stats: { totalUsers: 0, totalVolume: 0, totalQuestions: 0, totalTrades: 0 }, allCategories: [] };
    }

    // Filter opinions by category if filter is applied
    const filteredOpinions = categoryFilter
      ? opinions.filter(op => op.categories?.some(cat =>
    ...
    opinions.forEach(op => {
      op.categories?.forEach(cat => allCategoriesSet.add(cat));
    });
    ...
    const userMap = new Map<string, {
      ...
      tradesCount: number;
      categories: Set<string>;     // Categories user is active in
    }>();
    ...
    filteredOpinions.forEach((opinion) => {
      ...
      try {
        ...
        const opinionCategories = opinion.categories || [];
      ...
      opinionCategories.forEach(cat => creatorData.categories.add(cat));
      ...
      if (currentOwner && currentOwner !== creator) {
        ...
        // Track categories for this user
        opinionCategories.forEach(cat => ownerData.categories.add(cat));
    ...
    users.sort((a, b) => {
      switch (rankingType) {
        case 'roi':
          return b.trueROI - a.trueROI;
        case 'volume':
          return b.volumeGenerated - a.volumeGenerated;
        case 'questions':
    ...
    const stats: LeaderboardStats = {
      ...
      totalQuestions: categoryFilter ? filteredOpinions.length : totalOpinions - 1,
      totalTrades: totalTrades,
    };

    return { users, stats, allCategories };
  }, [opinions, totalOpinions, categoryFilter, rankingType]);

  return {
    users: leaderboardData.users,
    stats: leaderboardData.stats,
    allCategories: leaderboardData.allCategories,
...
export function useEnhancedLeaderboardData(filters?: LeaderboardFilters) {
  const baseData = useLeaderboardData(filters);
  ...
  const enhancedUsers = useMemo(() => {
    ...
    return baseData.users.map(user => {
      ...
      const actualVolumeGenerated = user.volumeGenerated || (user.creatorFees > 0 ? user.creatorFees / 0.03 : 0);
  ...
  return {
    ...
    allCategories: baseData.allCategories,
