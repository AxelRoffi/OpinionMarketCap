              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start gap-2">
                    <span className="text-muted-foreground font-mono text-[10px] mt-0.5 flex-shrink-0">#{opinion.id}</span>
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {displayCategories.slice(0, 2).map((category, catIndex) => (
                      <Badge
                        key={catIndex}
                        className={`${getCategoryColor(category)} cursor-pointer transition-colors duration-200 px-1.5 py-0 rounded text-[9px] font-medium leading-tight`}
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {/* Row 3: Answer + Price + Trade */}
                  <div className="flex items-center gap-2">
                    <button
                      className="text-foreground font-semibold text-sm leading-tight hover:text-blue-400 transition-colors flex items-center gap-1 bg-transparent border-none p-0 text-left focus:outline-none truncate min-w-0 flex-1"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground text-[10px]">
                    <span>
                      by <ClickableAddress
                        ...
                        className="text-muted-foreground hover:text-blue-400 cursor-pointer transition-colors"
                      >
                    ...
                    <span>· {accurateTradesCount || opinion.tradesCount || 0} trades</span>
                    <span className="ml-auto">Vol {formatUSDC(opinion.totalVolume)}</span>
                  </div>
                </motion.div>
              );
            })}
