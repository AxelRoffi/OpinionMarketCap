...
export function useQuestions(options?: { limit?: number; offset?: number }) {
  ...
  const questions: (Question & {
    ...
    leadingPricePerShare?: bigint;
    leadingTotalShares?: bigint;
  })[] = [];
  ...
  if (questionsData) {
    questionsData.forEach((result, index) => {
      if (result.status === 'success' && result.result) {
        ...
        let leadingPricePerShare: bigint | undefined;
        let leadingTotalShares: bigint | undefined;
        if (answerDetail?.status === 'success' && answerDetail.result) {
          ...
          leadingTotalShares = answerData[6]; // totalShares
          leadingPricePerShare = answerData[8]; // pricePerShare
        }
